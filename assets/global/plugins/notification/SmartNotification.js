...
$.sound_path = "../assets/global/sounds/";
...
(function ($) {
    $.SmartMessageBox = function (settings, callback) {
        ...
        settings = $.extend({
            ...
            buttons: undefined,
            buttonsPosition: undefined,
            buttonClass: undefined,
            buttonsIcon: undefined,
            input: undefined,
        ...
        if (isIE8orlower() == 0) {
            ...
            audioElement.setAttribute('src', PECO.base_url()+'assets/global/sounds/messagebox.mp3');
            $.get();
        ...
        if (settings.input != undefined) {
            ...
            switch (settings.input) {
                case "text":
                    ...
                    // LUCKY JOHN FADERON
                    // 04-17-2017
                    if(settings.inputIcon!= undefined) {
                        if(settings.inputIconPosition != undefined) {
                            inputPos = settings.inputIconPosition;
                        }else{
                            inputPos = 'left';
                        }
                        InputType = "<div class='input-icon "+inputPos+"'>" +
                            "<i class='"+settings.inputIcon+"'></i>" +
                            "<input class='form-control' type='" + settings.input + "' id='txt" +
                            SmartMSGboxCount + "' placeholder='" + settings.placeholder + "' value='" + settings.inputValue + "'/></div><br/><br/>";
                    }else {
                ...
                case "password":
                    ...
                    if(settings.inputIcon!= undefined) {
                        ...
                        InputType = "<div class='input-icon "+inputPos+"'>" +
                            "<i class='"+settings.inputIcon+"'></i>" +
                            "<input class='form-control' type='" + settings.input + "' id='txt" +
                            SmartMSGboxCount + "' placeholder='" + settings.placeholder + "'/></div><br/><br/>";
                    }else {
                        InputType = "<input class='form-control' type='" + settings.input + "' id='txt" +
        ...
        if(settings.buttonsPosition!=undefined) {
            buttonPos = settings.buttonsPosition;
        }else{
            buttonPos = "left";
        }
        ...
        Content += "<div class='input-group'>"+InputType+"</div>";
        Content += "<div class='MessageBoxButtonSection "+buttonPos+"'>";
        ...
        if (settings.cornerType == undefined) {
            var btn_corner = '';
        }else{
            var btn_corner = settings.cornerType;
        }
        ...
        for (var i = 0; i <= settings.buttons.length - 1; i++) {
            ...
            if (settings.buttons[i] == "[") {
                ...
                if (settings.buttons[i] == "]") {
                    ...
                    /*
                     @FIXED
                     ADDED FEATURES FOR BUTTON COLOR
                     Author: Lucky John F. Faderon
                     Date: 04-12-2017
                     */

                    if(settings.buttonClass!=undefined) {
                        settings.buttonClass = $.trim(settings.buttonClass);
                        settings.buttonClass = settings.buttonClass.split(',');
                        var class_num = NumBottons - 1;
                        if (settings.buttonClass[class_num]) {
                            var btn_class = settings.buttonClass[class_num];
                        }else{
                            var btn_class = 'btn-default';
                        }
                    }
                    ...
                    /*
                     @FIXED
                     ADDED FEATURES FOR ICONS
                     Author: Lucky John F. Faderon
                     Date: 04-12-2017
                     */
                    if(settings.buttonsIcon!=undefined) {
                        settings.buttonsIcon = $.trim(settings.buttonsIcon);
                        settings.buttonsIcon = settings.buttonsIcon.split(',');
                        var btn_icon_num = NumBottons - 1;
                        if (settings.buttonsIcon[btn_icon_num]) {
                            var btn_icon_fa_content = settings.buttonsIcon[btn_icon_num];
                            var btn_icon = true;
                        }
                    }
                    if(btn_icon==true) {
                        var btn_icon_fa = '<i class="fa '+btn_icon_fa_content+'"></i> ';
                    }

                    Name = "<button id='bot" + NumBottons + "-Msg" + SmartMSGboxCount +
                        "' class='btn "+btn_class+" btn-sm "+btn_corner+" botTempo'> " + btn_icon_fa + " " + Name + "</button>";
                    Content += Name;
                } else {
        ...
        if (settings.sound === true) {
            ...
            if (isIE8orlower() == 0) {
                ...
                if (navigator.userAgent.match('Firefox/'))
                    audioElement.setAttribute('src', PECO.base_url()+'assets/global/sounds/'+sound_url+'.ogg');
                else
                    audioElement.setAttribute('src', PECO.base_url()+'assets/global/sounds/'+sound_url+'.mp3');
