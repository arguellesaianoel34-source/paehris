...
<div class="row">
    <div class="col-md-12">
        ...
        $req_code = $this->db->select('prp.codes')
            ->from('prime_requirement_parameters as prp')
            ->join('application_customers_requirements as cr','prp.sysid = cr.reqid','left')
            ->where(array('cr.sysid' => $ids,'cr.status' => 1))
            ->get()->row();

        $dir = "uploads/attachments/cad/applications/".$dataid."/requirements/".$req_code->codes."*.*";
