...
<link rel="stylesheet" type="text/css" href="<?php echo base_url(); ?>assets/global/plugins/datatables/extensions/FixedColumns/css/dataTables.fixedColumns.css"/>
        ...
        <div class="portlet box tabbable">
            <div class="portlet-title">
                <div class="caption">
                    <span class="caption-subject font-green-sharp bold uppercase">Budget Request (PCEO View New)</span>
                                ...
                                <div class="portlet table light">
                                    ...
                                    <div class="portlet-body">
                                        <ul class="list-group summary no-border list-group-sm">
                                            ...
                                            <li class="list-group-item"> Total Amount Proposed: <span class="label label-default pull-right" id="proptotal">0</span> </li>
...
<script>
    ...
    $('body').on('change', '.budget_comment', function () {
        var budgetDataId = $(this).closest('tr').find('#budgetid').val();
        alert("changed");
        //TODO change this to the person that was currently logged.
        var currentLogged = 1;
        var comment = $(this).val();
        $.ajax({
            url: PECO.base_url() + 'bos/addRemarks',
            type: 'post',
            data: {'budgetDataId': budgetDataId, 'currentLogged': currentLogged, 'comment':comment},
            dataType: 'json'
        }).done(function (d) {
            PECO.initAlerts(d.msg, 'Information', d.func);
            viewNewlyInserted(budgetDataId, d.lastId);
        }).fail(function () {
            PECO.PhpError();
        });
    });
    ...
    function createdatatable(tableid, tfootid) {
        ...
        var powerpriv = 1;//1 for pceo disapproval button.
        $.ajax({
            ...
            $('body').find('#proptotal').text(data.proptotal);
