...
$cadtrans = $this->db->select('sysid')
    ...
    ->where(array('flowid'=>$flowid,'desc'=>'CAD-Transmital','status'=>1))
    ->get()->row();
...
$stage = $this->db->select('trmt.*')
    ->from('transaction_request_main_trails as trmt')
    ->where('trmt.dataid',$dataid)
    ->order_by('trmt.datecreated','DESC')
    ->get()->row();

$search_ecales = $this->db->select()->from('customer_ecales_logs')
    ->where(array('dataid' => $dataid , 'status' => 314))
    ->get()->row();

$hasgdr = $this->db->select()
    ->from('application_customers_charges')
    ->where(array('appid' => $dataid , 'chargeid' => 162 , 'status != ' => 0))
    ->get()->row();
?>
...
<script type="text/javascript">
    //GMAPSMAIN.mapping(<?php echo $dataid; ?>, 'custmap', true, <?php echo $moduleid; ?>);
    //FormEditable.init();
