...
$payrolldataid = $dataid;

$checkifalreadyapprove = $this->db->select("status")->from("payroll_reports_group")->where(array("sysid" => $payrolldataid))->get()->row();
if($checkifalreadyapprove){

    if($checkifalreadyapprove->status == 1){

        echo '<div class="btn-group pull-right pceopayrollbtn" style="margin-top: 20px !important;">
                <button id="approvebtn" class="btn btn-primary"><i class="fa fa-check"></i> Approve</button>
                <button id="disapprovebtn" class="btn btn-danger"><i class="fa fa-times"></i> Disapprove</button>
            </div>';
    }
}
