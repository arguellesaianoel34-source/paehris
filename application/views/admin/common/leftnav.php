<?php
$menu_preference = $this->db->select()
    ->from('prime_system_users_preferences')
    ->where(array('userid' => user_id(),'status' => 1))
    ->get()->row();

$scroll_class = 'menu-scroll';
if ($menu_preference) {
    if ($menu_preference->menutype == 0) {
        $scroll_class = '';
    }
}
...
<div class="page-sidebar-wrapper">
    ...
    <div class="page-sidebar navbar-collapse collapse" data-auto-speed="300" data-auto-scroll="true">
        ...
        <ul class="page-sidebar-menu page-sidebar-menu-compact page-sidebar-menu-hover-submenu <?php echo $scroll_class; ?>" data-keep-expanded="TRUE" data-auto-scroll="true" data-slide-speed="200">
            ...
            if (super_admin() || ($menu_preference && $menu_preference->menutype == 0)) {
            ...
            ')
                ...
                $qry_nav = $this->db->select()
                    ->from('prime_module_navigations_main')
                    ->where_in('sysid',$user_nav)
                    ->where(array('url IS NOT NULL' => null, 'status' => 1))
                    ->get();

                if ($qry_nav->num_rows() > 0) {
                    foreach ($qry_nav->result() as $nav) {
                        $sub_menu_class = $this->model_admin->init_navigation_open_sub($this->uri->segment(2), $nav->sysid);
                        $navhash = $nav->hashcode;
                        $url_str = ($nav->url!='') ? '/' . $nav->url : '';
                        $url = 'module/' . $navhash . $url_str;
                        $html = '';
                        $html .= '<li class="nav-item ' . $sub_menu_class->class . '">';
                        $html .= '<a href="'.base_url($url).'"> <i class="fa ' . $nav->icon . ' text-' . $nav->htmlclass . '"></i> <span class="title">' . $nav->name . '</span><span class="arrow"></span></a>';
                        $html .= '</li>';
